...
def iterate_centroids(xs, centroids):
    ...
    err = 1  # minimum amount of allowed centroid movement
    observed_error = 2  # Initialize: maxiumum amount of centroid movement
...
def find_centroids_clusters(dataset, k):
    try:
        initial_centroids = pick_centroids(dataset, k)
        final_centroids, final_clusters = iterate_centroids(dataset, initial_centroids)
        return ("success", final_centroids, final_clusters)
    except:
        return ("error",[],[])
    ## pair the centroid
    #return zip(final_centroids, final_clusters)
